...
describe('Etsy.listings', function() {
    before(function() {
        etsy = new Etsy();
    });
    ...
    describe('#get()', function() {
        context('when using default options', function() {
            it('should create output/listings.json', function(done) {
                this.timeout(5000);

                etsy.listings.get().then((response) => {
                    return response.text();
                }).then((text) => {
                  // make a file system agnostic file name
                  let fileName = 'output' + path.sep + 'listings.json';
            ...
            it('should create output/listings.csv', function(done) {
                this.timeout(5000);
                ...
                etsy.listings.get().then((response) => {
                    ...
                    // write the csv file
                    let fileName = 'output' + path.sep + 'listings.csv';
        ...
        context('when using pagination options', function() {
            it('should get two pages and concat them', function(done) {
                ...
                etsy.listings.get(options).then((response) => {
                    return response.json();
                }).then((data) => {
                    ...
                    return etsy.listings.get(options);
                }).then((response) => {
                    return response.json();
                }).then((data) => {
                    rows = rows.concat(data.results);
